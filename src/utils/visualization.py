...
def plot_training_progress(
    ...
    # Previous plot is erased to update all information
    ...
    # Determine xticks positions based on the number of epochs. If number of
    # epochs/sessions is too big, only each 5-th tick is used.
    if len(logs) > 11:
    ...
    for i in range(len(logs[-1]["metrics"])):
        # Each metric int logs[i]["metrics"] is a list[str, float], where str -
        # is a name and float is the metric's value.
        ax2.plot(
            [epoch["metrics"][i][1] for epoch in logs],
            # The colors become brighter (greener) as the index of the metric increases
